...
class FeedUIIntegrationTests: XCTestCase {
    ...
    func test_loadMoreAction_requestMoreFromLoader() {
        let (sut, loader) = makeSUT()
        sut.loadViewIfNeeded()
    ...
    func test_loadFeedCompletion_rendersErrorMessageOnFailureUntilFeedRefresh() {
        let (sut, loader) = makeSUT()
    ...
    func test_tapOnLoadMoreErrorView_loadsMore() {
        ...
        sut.simulateTapOnLoadMoreFeedError() // We should not have an error at the moment, so it should do anything
        XCTAssertEqual(loader.loadMoreCallCount, 1)
    ...
    private func makeImage(description: String? = nil, location: String? = nil, url: URL = URL(string: "http://any-url.com")!) -> FeedImage {
        return FeedImage(id: UUID(), url: url, location: location, description: description)
    }
