...
class ValidateFeedCacheUseCaseTests: XCTestCase {
    ...
    private func expect(_ sut: LocalFeedLoader, toCompleteWith expectedResult: Result<Void, Error>, when action: () -> Void, file: StaticString = #filePath, line: UInt = #line) {
        ...
        let validationError = Result { try sut.validateCache() }
        
        switch (validationError, expectedResult) {
        ...
        default:
            XCTFail("Expected result \(expectedResult), got \(validationError) instead", file: file, line: line)
        }
    }
}
